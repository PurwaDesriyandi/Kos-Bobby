<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['name']) && isset($_POST['phone_number']) && isset($_POST['address']) && isset($_POST['date']) && isset($_POST['rent_duration']) && isset($_POST['nokamar']) && isset($_POST['total_harga'])) {
        ...
        if(empty($nama) || empty($nomor_hp) || empty($alamat) || empty($fasilitas) || empty($tanggal) || empty($sewa_bulan) || empty($no_kamar)){
            $errorMessage = "<h4 style='color: #595f3a; text-align: center; font-size:27px;'>Silahkan Isi Semua Kolom Yang Ada.</h4>";
        } else {
...
<body>
    ...
    <section>
        ...
        <div class="container" style="max-width:1000px; padding: 40px 100px; border-radius: 40px;  background-color: #ffffff;">
            <form method="POST" class="form-horizontal" role="form" id="rentalForm">
                ...
                <div class="form-group">
                    <label for="facilities" class="col-sm-4">Barang Elektronik Yang Dibawa</label>
                    <div class="col-sm-9">
                        <div class="checkbox">
                ...
                <div class="form-group">
                    ...
                    <button type="submit button" id="whatsappButton" class="btn btn-success btn-save" onclick="whatsappMessage()">Simpan</button>
                    <button style="margin-left: 2%;"  type="button" class="btn btn-primary" onclick="total()">Total Harga</button>
    ...
    <script>
            ...
            document.querySelectorAll('input[type=checkbox]:checked').forEach(function(item) {
                total += parseInt(item.id) * rentDurationValue; 
            });
        ...
        function whatsappMessage() {
            ...
            if (!name || !phone_number || !address || !date || !total_sewa || !nokamar) {
                alert("Silahkan Isi Semua Kolom Yang Ada.");
                return; 
            }
            ...
            total();
            var total_price = document.getElementById('result').textContent.replace('Total Harga: Rp ', '').trim();

            var whatsappMessage = `*Pemesanan Kos Putri Griya Barokah 2*\n\n` +
                `*Nama:* ${name}\n` +
                `*No Handphone:* ${phone_number}\n` +
                `*Alamat Domisili:* ${address}\n` +
                `*Barang Elektronik Yang Dibawa:* ${additional_facilities_info.join(', ')}\n` +
                `*Tanggal Masuk:* ${date}\n` +
                `*Waktu Sewa:* ${total_sewa} Bulan = Rp. ${rent_duration} \n` +
                `*Pilih No Kamar:* ${nokamar}\n` +
                `*Total Harga Sewa: Rp. ${total_price}*`;

            var whatsappUrl = `https://wa.me/6289649955776?text=${encodeURIComponent(whatsappMessage)}`;
